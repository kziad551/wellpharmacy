...
if (is_post()) {
    ...
    if ($editing) {
        ...
        $data['id'] = $id;
        q("UPDATE brands SET name=:name, slug=:slug, color=:color, logo=:logo, featured=:featured, sort=:sort WHERE id=:id", $data);
        ...
        q("INSERT INTO brands (name,slug,color,logo,featured,sort) VALUES (:name,:slug,:color,:logo,:featured,:sort)", $data);
        flash($upErr ? 'Brand created — but no logo was added: ' . $upErr : 'Brand created.', $upErr ? 'err' : 'ok');
    }
    redirect('brands');
}

$v = $editing ? $b : ['id'=>0,'name'=>'','slug'=>'','color'=>'','logo'=>'','featured'=>0,'sort'=>0];
