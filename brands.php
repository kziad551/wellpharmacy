...
<style>
  ...
  .br-hero h1{font-family:var(--fp);font-size:clamp(32px,4vw,48px);font-weight:600;margin:10px 0 8px;letter-spacing:-.02em}
  .br-hero .sub{color:var(--ink-soft);font-size:16px;max-width:56ch;margin-inline:auto}
  .brandgrid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:18px;padding-block:36px 12px}
  ...
  @media(max-width:980px){.brandgrid{grid-template-columns:repeat(3,minmax(0,1fr))}}
  @media(max-width:620px){
    .brandgrid{grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}
    /* long names ("La Roche-Posay") must wrap inside the card, not widen the grid */
